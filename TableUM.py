...
def button_name(app):
    ...
    for i in range(0,len(app.but_list)): # i in 0..9
        if i != rand_but:
            ...
            val = des_val
            app.but_list[i].configure(text=val, command=lambda: next_ex(app), bg=app.btn_std_col)
        list_val.append(val)
...
def init_APP(list_of_num, list_counts):
    ...
    app.progr_bar = ttk.Progressbar(app,
                                    length=900,
                                    variable=app.count_task,
                                    maximum=len(app.list_counts)+1
                                    )
